...
export const processGameResults = (
  ...
  const updatePlayerStats = (player: Player, teamName: string, isHomeTeam: boolean) => {
    let currentSeasonStat = player.currentStats.find(s => s.season === seasonString && s.team === teamName && s.league === (isNationalsGame ? 'Nationals' : (isHomeTeam ? userTeam.leagueDivision : opponentTeam.leagueDivision)));
