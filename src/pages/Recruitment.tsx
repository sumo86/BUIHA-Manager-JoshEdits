...
const Recruitment = () => {
  const { userTeam, scoutingPool, recruitedPool, transferPool, fairHosted, generateScoutingPool } = useTeam();
  ...
  // Conditional rendering for when no players are available and fair hasn't been hosted
  if (!userTeam) {
    return (
      <div>
        <h1 className="text-3xl font-bold mb-4">Recruitment</h1>
        <p className="text-lg text-muted-foreground">
          Please select a team to manage to access recruitment features.
        </p>
      </div>
    );
  }

  const showNoPlayersMessage = scoutingPool?.length === 0 && recruitedPool?.length === 0 && transferPool?.length === 0;
  ...
  return (
    <div className="space-y-4">
      ...
      {showNoPlayersMessage && !fairHosted ? (
        <Card className="p-6 text-center">
          <CardTitle className="mb-4">No Players to Scout</CardTitle>
          <CardDescription className="mb-4">
            The scouting pool is currently empty. Host the student fair to find new talent for your team!
          </CardDescription>
          <Button onClick={generateScoutingPool}>Host Student Fair</Button>
        </Card>
      ) : showNoPlayersMessage && fairHosted ? (
        <Card className="p-6 text-center">
          <CardTitle className="mb-4">Scouting Pool Empty</CardTitle>
          <CardDescription>
            You have already hosted the student fair for this season. Recruit players from your current pool or check back next season for new talent.
          </CardDescription>
        </Card>
      ) : null}
