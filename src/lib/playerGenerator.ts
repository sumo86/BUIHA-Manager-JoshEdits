  ...
  if (isSkater) {
    if (Math.random() > 0.5) { 
      ...
      while (true) {
        ...
        if (!positions.some((p: Position) => p === (secondaryPosition as Position))) { // Explicit cast here
          break;
        }
      }
    ...
    if (positions.length === 2 && Math.random() > 0.8) { 
      ...
      while (true) {
        ...
        if (!positions.some((p: Position) => p === (tertiaryPosition as Position))) { // Explicit cast here
          break;
        }
      }
