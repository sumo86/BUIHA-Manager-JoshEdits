...
import { Player, Position, SkaterAttributes, GoalieAttributes, PlayerArchetype, PlayerSeasonStats } from '@/types';
...
const generatePlayerHistory = (playerAge: number, isSkater: boolean, teamName: string, leagueDivision: string): PlayerSeasonStats[] => {
  const history: PlayerSeasonStats[] = [];
  const yearsOfHistory = playerAge - 18;
  if (yearsOfHistory <= 0) return [];

  const currentYear = new Date().getFullYear();

  for (let i = 0; i < yearsOfHistory; i++) {
    const seasonYear = currentYear - (yearsOfHistory - i);
    const seasonString = `${seasonYear - 1}-${seasonYear}`;
    const gamesPlayed = getRandomNumber(10, 24);
    ...
    if (isSkater) {
      const goals = getRandomNumber(0, Math.floor(gamesPlayed * 1.2));
      const assists = getRandomNumber(0, Math.floor(gamesPlayed * 1.5));
      seasonStats = {
        ...seasonStats,
        goals,
        assists,
        points: goals + assists,
        penaltyMinutes: getRandomNumber(0, gamesPlayed * 3),
      };
    } else { // Goalie
      seasonStats = {
        ...seasonStats,
        goalsAgainstAverage: parseFloat((Math.random() * (4.50 - 1.80) + 1.80).toFixed(2)),
        savePercentage: parseFloat((Math.random() * (0.925 - 0.870) + 0.870).toFixed(3)),
        shutouts: getRandomNumber(0, Math.floor(gamesPlayed / 6)),
      };
    }
...
export const generatePlayer = (usedJerseyNumbers: Set<number>, position: Position, leagueDivision: string, teamName: string, allowedEligibilities?: Player['eligibility'][], options?: { targetStarRating?: number, targetAbility?: number }): Player => {
  ...
  const history = generatePlayerHistory(age, isSkater, teamName, leagueDivision);
