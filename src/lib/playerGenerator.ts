...
import { getRandomNationality } from '@/data/nationalityDistributions'; // Corrected import
// Removed: import { abilityRanges } from '@/data/abilityRanges';
import { starRatingDistribution } from '@/data/starRatingDistribution'; // Corrected import
...
export const calculateStarRating = (ability: number, isSkater: boolean, leagueDivision: string): number => {
  const distribution = starRatingDistribution[leagueDivision]; // Corrected usage
...
export const generatePlayer = (usedJerseyNumbers: Set<number>, position: Position, leagueDivision: string, teamName: string, allowedEligibilities?: Player['eligibility'][], options?: { targetStarRating?: number, targetAbility?: number }): Player => {
  ...
  const nationality = getRandomNationality(); // Corrected usage
  ...
  return {
    ...
    activeInstructions: [],
    jerseyNumber, // Ensure jerseyNumber is included here
  };
};
...
export const generateRecruits = (leagueDivision: string, existingTeamNames: string[]): Player[] => {
  ...
  const allRecruitPositions: Position[] = ['C', 'LW', 'RW', 'LD', 'RD', 'G']; // Explicitly typed array
  for (let i = 0; i < numRecruits; i++) {
    const randomPosition: Position = getRandomItem(allRecruitPositions);
